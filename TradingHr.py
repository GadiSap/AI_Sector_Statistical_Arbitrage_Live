...
class TradingHr:
    ...
    def __init__(self, trades_history_file_name='trades.csv', time_series_file_name='trade_time_series.csv', fee=0.005,
                 entry_threshold=1.5, exit_threshold=0.2, stop_loss_threshold=4.5, enter_trade_max=3.0):
        """
        ...
        if not os.path.exists(self.time_series_file_name):
            pd.DataFrame(columns=['Date', 'Ticker1', 'Ticker2', 'Z-score', 'Pair Profit', 'Intrade Status',
                                  'Current Trade PnL']).to_csv(self.time_series_file_name, index=False)

    def sell(self, enter_price_stock1: float, enter_price_stock2: float, price_stock1: float, price_stock2: float,
    ...
    def trading(self, data_training: pd.DataFrame):
        ...
        for i in range(df_trade_history.shape[0]):
            ...
            if data_training[ticker1].isnull().all() or data_training[ticker2].isnull().all() or len(
                    data_training[ticker1].dropna()) < 2 or len(data_training[ticker2].dropna()) < 2:
                print(
                    f"Skipping pair {ticker1}/{ticker2} for this period due to insufficient valid data for OLS or price access.")
            ...
            try:
                ...
                print(
                    f"ERROR: Unexpected KeyError when accessing prices for {ticker1}/{ticker2} at .iloc[-1][ticker]. Error: {e}. Skipping pair for this period.")
                time_series_records_for_this_day.append({
                    'Date': current_date,
                    'Ticker1': ticker1,
                    'Ticker2': ticker2,
            ...
            if row['intrade'] == 'yes':
                ...
                unrealized_profit_current_trade = self.sell(enter_price_stock1, enter_price_stock2, price_stock1,
                                                            price_stock2, hedge_ratio)
                current_trade_pnl = unrealized_profit_current_trade  # Record for time series

                # Calculate the *total compounded profit for the pair* if this open trade were closed now.
                # This value is for the time series recording, reflecting total performance including unrealized.
                if cumulative_realized_profit == 0.0:
                    current_pair_profit_for_timeseries = unrealized_profit_current_trade
                else:
                    current_pair_profit_for_timeseries = (1 + cumulative_realized_profit) * (
                                1 + unrealized_profit_current_trade) - 1
                ...
                if abs(current_z_score) <= self.exit_threshold or abs(current_z_score) >= self.stop_loss_threshold:
                    # Trade is being closed. Update the *realized* cumulative profit for the pair by compounding.
                    if row['profit'] == 0.0:
                    ...
                    if row['Two Months Profit'] == 0.0:
                        ...
                        row['Two Months Profit'] = (1 + row['Two Months Profit']) * (
                                    1 + unrealized_profit_current_trade) - 1
            ...
            time_series_records_for_this_day.append({
                ...
                'Pair Profit': current_pair_profit_for_timeseries,
                # This is the compounded cumulative profit for the pair up to this point
                'Intrade Status': row['intrade'],
