...
class TradingHr:
  ...
  """
  def __init__(self, trades_history_file_name = 'trades.csv', time_series_file_name = 'trade_time_series.csv', fee = 0.005, entry_threshold = 1.5, exit_threshold = 0.2, stop_loss_threshold = 4.5, enter_trade_max = 3.0):
    """
  ...
  def trading(self, data_training: pd.DataFrame):
    ...
    for i in range(df_trade_history.shape[0]):
      ...
      if row['intrade'] == 'yes':
        ...
        enter_price_stock1 = row['ticker1_buy_price']
        enter_price_stock2 = row['ticker2_buy_price']
        unrealized_profit_current_trade = self.sell(enter_price_stock1, enter_price_stock2, price_stock1, price_stock2, hedge_ratio)
      ...
      time_series_records_for_this_day.append({
          ...
          'Pair Profit': current_pair_profit_for_timeseries, # This is the compounded cumulative profit for the pair up to this point
          'Intrade Status': row['intrade'],
